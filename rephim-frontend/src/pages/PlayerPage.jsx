...
export default function PlayerPage() {
  ...
  const toggleFullscreen = () => {
    ...
    const requestNativeFullscreen = async (el) => {
      ...
      try {
        const result = requestFn({ navigationUI: "hide" });
        ...
        try {
          const retryResult = requestFn();
          if (retryResult && typeof retryResult.then === "function") {
            await retryResult;
          }
          return true;
        } catch (secondError) {
          console.warn("Native fullscreen failed:", secondError || firstError);
          return false;
        }
      }
    };
  ...
  return (
    <div className="w-full bg-[#0e0e0e] min-h-[50vh] flex flex-col pb-12">
      {/* Player container */}
      <div
        ref={containerRef}
        className={
          fullscreen
            ? "player-container fixed inset-0 z-9999 bg-black"
            : "player-container w-full max-w-5xl mx-auto aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative mt-6"
        }
      ...
      >
        ...
        <div className="relative w-full h-full flex items-center justify-center bg-black">
          ...
          <div
            className={`absolute inset-0 flex items-center justify-center gap-8 bg-black/40 transition-opacity duration-300 z-20 pointer-events-none ${controlsVisible ? "opacity-100" : "opacity-0"
              }`}
          >
            {/* Skip Back Button */}
            <button
        ...
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-white/5">
            ...
            <div className="flex gap-3 shrink-0">
              <Link
                to={`/phim/${slug}`}
                className="glass hover:bg-white/10 text-sm font-semibold px-5 py-2.5 rounded-full transition-all text-gray-300"
              >
                ← Thông tin phim
              </Link>
            </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {episodes.map((server, i) => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${i === activeServer
              ...
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                {serverEps.map((ep, i) => (
                  <button
                    ...
                    className={`py-2 px-1 rounded-lg text-center text-xs font-semibold transition-all ${currentEp?.slug === ep.slug
                      ? "bg-[#E50914] text-white"
                      : "glass glass-hover text-gray-300 hover:text-white hover:bg-[#E50914]/20"
                      }`}
                  >
                    {ep.name}
                  </button>
